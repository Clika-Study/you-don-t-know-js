...
- `softBind()` 유틸리티는 호출 시점에 `this`를 체크하는 부분에서 주어진 함수를 래핑하여 전역 객체나 `undefined`일 경우엔 미리 준비한 대체 기본 객체로 세팅한다.
  ...
  ```
  - 소프트 바인딩이 탑재된 `foo` 함수는 `this`를 `obj2`나 `obj3`으로 수동 바인딩할 수 있고, 기본 바인딩 규칙이 적용되어야 할 땐 다시 `obj`으로 되돌린다.
## 어휘적 this
- 일반적인 함수는 지금까지 살펴본 4가지 규칙을 준수한다. 하지만! **화살표 함수**는 4가지 표준 규칙 대신 에두른 스코프를 보고 `this`를 알아서 바인딘한다.
- 이는 `bind()` 대신 사용가능한 해결책이지만 `this`에서 도망치려는 꼼수다(라고 저자는 말한다)
